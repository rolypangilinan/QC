...
class FluctuationMonitor:
    def __init__(self, root, location, generate_csv, tolerance, file_path, logic):
        ...
        ttk.Separator(self.details_frame, orient='horizontal').pack(fill='x', pady=5)
        self.logic_label = ttk.Label(self.details_frame, text=f"LOGICAL COMPUTATION: {logic}", 
                                     font=('Arial', 12, 'bold'))
        self.logic_label.pack(anchor='w')
        self.avg_frame = None
        self.avg_vars = {}
        self.current_avgs = {}
    ...
    def process_and_update(self):
        ...
        try:
            ...
            for a in range(len(df)):
                ...
                fluctuations = {}
                ref_serial = "N/A"
                if self.logic in ["FIND THE NEAREST GOOD", "AKH (DOUBLE NOZZLE)", "DUO (SINGLE NOZZLE) W/ 2 SERIAL NUMBER"]:
                ...
                elif self.logic == "ACCU AVG (TOL 5%)":
