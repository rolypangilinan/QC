...
class DatabaseSelection:
    def __init__(self, root):
        ...
        self.root.geometry("600x900")                     # 1ST TKINTER GUI WINDOW SIZE
...
class FluctuationMonitor:
    def __init__(self, root, location, generate_csv, tolerance, file_path, logic):
        ...
        if logic == "ACCU AVG (TOL 5%)":
            ...
            self.ref_serial_display.pack_forget()
        ...
        if self.logic == "AKH (DOUBLE NOZZLE)":
            self.last_good_values_per_model = {}
            self.last_good_serial_per_model = {}
        if self.logic == "DUO (SINGLE NOZZLE) W/ 2 SERIAL NUMBER":
            self.serial_to_runs = {}
            self.last_good_first = None
            self.last_good_second = None
        ...
        try:
            ...
            for a in range(len(df)):
                ...
                if self.logic == "ACCU AVG (TOL 5%)":
                    ...
                    current_dt = tempdf["DATETIME"].values[0]
                    if model_changed or is_new_date or is_new_model_in_day:
                        self.serial_to_runs = {}
                        self.last_good_first = None
                        self.last_good_second = None
                    ...
                    ref_str = "N/A"
                    if run_number == 1:
                        if self.last_good_first:
                            ref_values = self.last_good_first['values']
                            ref_str = self.last_good_first['ref_str']
                    elif run_number == 2:
                        if self.last_good_second:
                            ref_values = self.last_good_second['values']
                            ref_str = self.last_good_second['ref_str']
                    if ref_values is None:
                        ...
                        for key in current_values:
                            if current_values[key] == 0 or ref_values[key] == 0:
                                ...
                                fluctuations[key] = abs((ref_values[key] - current_values[key]) / ref_values[key])
                    has_fluct = any(v > self.threshold for v in fluctuations.values())
                    if not has_fluct:
                        new_ref_str = f"{current_serial}_{current_time}"
                        if run_number == 1:
                            self.last_good_first = {'values': current_values.copy(), 'ref_str': new_ref_str}
                        elif run_number == 2:
                            self.last_good_second = {'values': current_values.copy(), 'ref_str': new_ref_str}
                    ref_serial = ref_str
                    self.ref_serial_display.config(text=f"PREVIOUS REF SN: {ref_serial}")
                    ...
                    if is_new_date:
                        self.last_good_values_per_model = {}
                        self.last_good_serial_per_model = {}
                    has_last = model in self.last_good_values_per_model and bool(self.last_good_values_per_model[model])
                    if not has_last:
                        for key in current_values:
                            fluctuations[key] = 0
                        ref_serial = "N/A"
                    else:
                        last_values = self.last_good_values_per_model[model]
                        for key in current_values:
                            if current_values[key] == 0:
                                fluctuations[key] = 0
                            else:
                                fluctuations[key] = abs((last_values[key] - current_values[key]) / last_values[key]) if last_values[key] != 0 else 0
                        ref_serial = self.last_good_serial_per_model[model]
                    self.ref_serial_display.config(text=f"PREVIOUS REF SN: {ref_serial}")
                    if all(v <= self.threshold for v in fluctuations.values()):
                    ...
                    for key in fluctuations:
                        if fluctuations[key] > self.threshold:
                            ...
                            if self.logic == "ACCU AVG (TOL 5%)":
                                ...
                                ref = self.last_good_values_per_model[model_code][key]
                            elif self.logic == "DUO (SINGLE NOZZLE) W/ 2 SERIAL NUMBER":
                                ref = ref_values[key]
                            else:
                                ref = self.last_good_values[key]
        ...
        try:
            ...
            if all(last_row[col] <= self.threshold for col in self.status_vars if col in last_row):
                if self.logic == "AKH (DOUBLE NOZZLE)":
                    ...
                    serial = last_row["SERIAL No."]
                    mask = (self.compiledFrame["SERIAL No."] == serial) & (self.compiledFrame["DATETIME"] <= last_row["DATETIME"])
                    run_number = len(self.compiledFrame[mask])
                    values = {
                        '50Hz_WATTAGE': last_row["50Hz WATTAGE"],
                        '50Hz_AIR_VOLUME': last_row["50Hz AIR VOLUME"],
                        '50Hz_CLOSED_PRESSURE': last_row["50Hz CLOSED PRESSURE"],
                    ...
                    ref_str = f"{serial}_{last_row['TIME']}"
                    if run_number == 1:
                        self.last_good_first = {'values': values, 'ref_str': ref_str}
                    elif run_number == 2:
                        self.last_good_second = {'values': values, 'ref_str': ref_str}
                else:
                    self.last_good_values = {
                        '50Hz_WATTAGE': last_row["50Hz WATTAGE"],
                        '50Hz_AIR_VOLUME': last_row["50Hz AIR VOLUME"],
