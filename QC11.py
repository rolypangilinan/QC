# WORKING
# WITH FREEZE PANE BEFORE SPC LINE TREND GRAPH
# BAR GRAPH HIDE HALF SCREEN
# CREATE A 4 SEPARATE SPC LINE CHART (REAL TIME)
# WITH HIDE BUTTON
...
class DatabaseSelection:
    def __init__(self, root):
        ...
        self.root.geometry("580x600+383+50")  # (WIDTHxHEIGHT+LeftRight+UpDown) Sets size to 600x600 and positions it 200 pixels from the left and top of the screen
        ...
        locations = [
            ("SWITCH 1 - ATU006", "HP1-ATU006"),
            ("SWITCH 2 - ATU003", "HP2-ATU003"),
            ("FAST-LINE - ATU007", "FASTLINE-ATU007"),
            ("MULTILINE - ATU005", "MULTILINE-ATU005"),
            ("TESTING", "TESTING")
        ]
...
# DROPDOWN LIST COMBOBOX 
        ...
        self.logic_var = tk.StringVar(value="ACCU AVG (TOL 5%)")  # CHANGED DEFAULT TO ACCU AVG
        logics = [
...
class FluctuationMonitor:
    def __init__(self, root, location, generate_csv, tolerance, file_path, logic):
        ...
        self.xaxis_var = tk.StringVar(value="Serial No")
        self.show_title_var = tk.BooleanVar(value=True)
        self.datapoints_var = tk.StringVar(value="Scatter")
        self.scatter_color_var = tk.StringVar(value="None")
        self.sample_number_var = tk.StringVar(value="others")
        self.sample_entry_var = tk.StringVar(value="20")
        self.layout_var = tk.StringVar(value="Horizontal")  # CHANGED TO HORIZONTAL
        self.legend_var = tk.StringVar(value="NO")
        self.hide_50hz = tk.BooleanVar(value=False)
        self.hide_60hz = tk.BooleanVar(value=False)
        ...
        self.fixed_top = ttk.Frame(self.main_container)
        self.fixed_top.pack(side="top", fill="x")
        self.scroll_container = ttk.Frame(self.main_container)
        ...
        self.root.bind_all("<Button-4>", self._on_mousewheel)
        self.root.bind_all("<Button-5>", self._on_mousewheel)
        back_button_frame = ttk.Frame(self.fixed_top)
        ...
        zoom_frame = ttk.Frame(self.fixed_top)
        zoom_frame.pack(pady=2, anchor='ne')
        ttk.Button(zoom_frame, text="Hide", command=self.open_hide_selection).pack(side=tk.LEFT, padx=2)
        ...
        self.top_frame = ttk.Frame(self.fixed_top)
        ...
        self.serial_sep = ttk.Separator(self.left_frame, orient='horizontal')
        self.serial_sep.pack(fill='x', pady=5)
        self.ref_serial_display = tk.Label(self.left_frame, text="REF SERIAL NO: N/A", 
                                         font=('Arial', 12), fg="green")
        if self.logic not in ["AKH (DOUBLE NOZZLE)", "ACCU AVG (TOL 5%)"]:
        ...
        self.sep_after_50 = ttk.Separator(self.details_frame, orient='horizontal')
        self.sep_after_50.pack(fill='x', pady=5)
        ...
        self.sep_after_60 = ttk.Separator(self.details_frame, orient='horizontal')
        self.sep_after_60.pack(fill='x', pady=5)
        self.avg_frame = None
        self.avg_vars = {}
        self.current_avgs = {}
        if logic in ["ACCU AVG (TOL 5%)", "AKH (DOUBLE NOZZLE)", "DUO (SINGLE NOZZLE) W/ 2 SERIAL NUMBER"]:
            ...
            self.avg_50hz_label = ttk.Label(self.avg_frame, text="50Hz AVERAGES:", font=('Arial', 10, 'bold'))
            self.avg_50hz_label.pack(anchor='w')
            self.avg_50hz_rows = []
            for col in avg_columns_50:
                ...
                self.avg_50hz_rows.append(row)
            self.avg_60hz_label = ttk.Label(self.avg_frame, text="60Hz AVERAGES:", font=('Arial', 10, 'bold'))
            self.avg_60hz_label.pack(anchor='w')
            self.avg_60hz_rows = []
            for col in avg_columns_60:
                ...
                self.avg_vars[col] = tk.StringVar(value="NONE")
                ttk.Label(row, textvariable=self.avg_vars[col]).pack(side=tk.LEFT)
                self.avg_60hz_rows.append(row)
            self.ref_serial_display.config(text="REF SERIAL NO: N/A")
        self.right_frame = ttk.Frame(self.top_frame)
        self.right_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)
        self.create_bar_graph()
        self.line_graph_frame = ttk.Frame(self.scrollable_frame)
        ...
        ttk.Button(button_frame, text="Display Settings", 
                  command=self.open_display_settings).pack(side=tk.LEFT, padx=2)
        ttk.Button(button_frame, text="Open All SPC Charts", 
                  command=self.open_all_spc_charts).pack(side=tk.LEFT, padx=2)
        fluct_log_frame = ttk.Frame(self.scrollable_frame)
    ...
    def zoom(self, factor):
        ...
        for widget, base_size in widgets:
            ...
            widget.config(font=(font_name, new_size))
        ...
        if hasattr(self, 'fig50'):
            self.fig50.set_size_inches(6 * self.zoom_level, 2 * self.zoom_level)
            self.canvas_graph50.draw()
        if hasattr(self, 'fig60'):
            self.fig60.set_size_inches(6 * self.zoom_level, 2 * self.zoom_level)
            self.canvas_graph60.draw()
    ...
    def create_bar_graph(self):
        ...
        self.fig50, self.ax50 = plt.subplots(figsize=(6, 2))
        self.ax50.set_title('50Hz Fluctuation Values')
        self.ax50.set_ylabel('Fluctuation Amount (%)')
        self.ax50.grid(True)
        self.fluctuation_measurements50 = [
            '50 WAT', '50 VOL', '50 CloP.', '50 AMP'
        ]
        self.fluctuation_values50 = [0] * len(self.fluctuation_measurements50)
        self.bars50 = self.ax50.bar(self.fluctuation_measurements50, self.fluctuation_values50, color='blue')
        # Set fixed ticks to avoid UserWarning
        self.ax50.set_xticks(range(len(self.fluctuation_measurements50)))
        self.ax50.set_xticklabels(self.fluctuation_measurements50, rotation=45, ha='right', fontsize=7)
        self.ax50.axhline(y=self.threshold * 100, color='r', linestyle='--', linewidth=1)
        self.ax50.text(1.01, self.threshold * 100, 'Threshold (%.1f%%)' % (self.threshold * 100), color='r', va='center', ha='left', 
                     transform=self.ax50.get_yaxis_transform(), fontsize=8, 
                     bbox=dict(facecolor='white', edgecolor='none', alpha=0.8))
        # Adjust layout with larger margins to avoid tight_layout warning
        self.fig50.subplots_adjust(left=0.15, right=0.85, top=0.85, bottom=0.3)
        self.canvas_graph50 = FigureCanvasTkAgg(self.fig50, master=self.right_frame)
        self.canvas_graph50.draw()
        self.canvas_graph50.get_tk_widget().pack(fill=tk.X, expand=False, padx=10, pady=5)
        self.fig60, self.ax60 = plt.subplots(figsize=(6, 2))
        self.ax60.set_title('60Hz Fluctuation Values')
        self.ax60.set_ylabel('Fluctuation Amount (%)')
        self.ax60.grid(True)
        self.fluctuation_measurements60 = [
            '60 WAT', '60 VOL', '60 CloP.', '60 AMP'
        ]
        self.fluctuation_values60 = [0] * len(self.fluctuation_measurements60)
        self.bars60 = self.ax60.bar(self.fluctuation_measurements60, self.fluctuation_values60, color='blue')
        # Set fixed ticks to avoid UserWarning
        self.ax60.set_xticks(range(len(self.fluctuation_measurements60)))
        self.ax60.set_xticklabels(self.fluctuation_measurements60, rotation=45, ha='right', fontsize=7)
        self.ax60.axhline(y=self.threshold * 100, color='r', linestyle='--', linewidth=1)
        self.ax60.text(1.01, self.threshold * 100, 'Threshold (%.1f%%)' % (self.threshold * 100), color='r', va='center', ha='left', 
                     transform=self.ax60.get_yaxis_transform(), fontsize=8, 
                     bbox=dict(facecolor='white', edgecolor='none', alpha=0.8))
        # Adjust layout with larger margins to avoid tight_layout warning
        self.fig60.subplots_adjust(left=0.15, right=0.85, top=0.85, bottom=0.3)
        self.canvas_graph60 = FigureCanvasTkAgg(self.fig60, master=self.right_frame)
        self.canvas_graph60.draw()
        self.canvas_graph60.get_tk_widget().pack(fill=tk.X, expand=False, padx=10, pady=5)

    def create_line_graph(self):
        """Create the matplotlib line graph for displaying historical trends"""
        self.line_fig = Figure(figsize=(14, 6), dpi=100)
        # Always use horizontal layout for 2 rows (50Hz and 60Hz)
        nrows = 2
        ncols = 4
        self.line_fig.set_size_inches(16, 6)
        self.line_axes = []
        for i in range(nrows * ncols):
            ax = self.line_fig.add_subplot(nrows, ncols, i + 1)
            ax.set_ylabel('Value')
            ax.grid(True)
            self.line_axes.append(ax)
    ...
    def update_line_graph(self):
        ...
        try:
            ...
            for ax in self.line_axes:
                ax.clear()
            if self.sample_number_var.get() == "others":
                try:
                    max_points = int(self.sample_entry_var.get())
                except:
                    max_points = 20
            else:
                max_points = int(self.sample_number_var.get())
            df = self.compiledFrame.tail(100)
            df = self.downsample_data(df, max_points=max_points)
            measurements = [
                ('50Hz WATTAGE', 'blue'),
                ('50Hz AIR VOLUME', 'green'),
                ('50Hz CLOSED PRESSURE', 'red'),
                ('50Hz AMPERAGE', 'violet'),
                ('60Hz WATTAGE', 'magenta'),
                ('60Hz AIR VOLUME', 'yellow'),
                ('60Hz CLOSED PRESSURE', 'black'),
                ('60Hz AMPERAGE', 'orange')
            ]
            color_map = {meas: color for meas, color in measurements}
            # Always use horizontal layout: 2 rows, 4 columns
            # First row: 50Hz measurements
            # Second row: 60Hz measurements
            meas_order = ['50Hz WATTAGE', '50Hz AIR VOLUME', '50Hz CLOSED PRESSURE', '50Hz AMPERAGE',
            ...
            # Handle x-axis display based on selection
            ...
            elif self.xaxis_var.get() == "None":  # ADDED NONE OPTION
                x = range(len(df))
                xticklabels = [''] * len(df)  # Empty labels
            else:
                x = df['DATETIME']
                xticklabels = []
                
            for idx, meas in enumerate(meas_order):
                ax = self.line_axes[idx]
                ax.clear()
                if ('50Hz' in meas and self.hide_50hz.get()) or ('60Hz' in meas and self.hide_60hz.get()):
                    ...
                    ucl = mean + 3 * std
                    lcl = mean - 3 * std
                    color = color_map[meas]
                    ax.plot(x, y, label=meas, color=color, linewidth=linewidth)
                    ax.axhline(mean, color=color, linestyle='-', alpha=0.5)
                    ax.axhline(ucl, color=color, linestyle='--', alpha=0.5)
                    ax.axhline(lcl, color=color, linestyle='--', alpha=0.5)
                    ...
                    # Handle x-axis display
                    if self.xaxis_var.get() != "None":  # Only show x-axis if not "None"
    ...
    def update_bar_graph(self, last_row):
        ...
        try:
            ...
            self.ax50.set_xticks(range(len(self.fluctuation_measurements50)))
            self.ax50.set_xticklabels(self.fluctuation_measurements50, rotation=45, ha='right', fontsize=7)
            max_value = max(self.fluctuation_values50) if max(self.fluctuation_values50) > 0 else 10
            self.ax50.set_ylim(0, max_value * 1.1)
            for bar in self.bars50:
                height = bar.get_height()
                self.ax50.text(bar.get_x() + bar.get_width()/2., height,
                            f'{height:.2f}',
                            ha='center', va='bottom')
            self.ax50.axhline(y=self.threshold * 100, color='r', linestyle='--', linewidth=1)
            self.ax50.text(1.01, self.threshold * 100, 'Threshold (%.1f%%)' % (self.threshold * 100), color='r', va='center', ha='left', 
            ...
            self.ax60.set_xticks(range(len(self.fluctuation_measurements60)))
            self.ax60.set_xticklabels(self.fluctuation_measurements60, rotation=45, ha='right', fontsize=7)
            max_value = max(self.fluctuation_values60) if max(self.fluctuation_values60) > 0 else 10
            self.ax60.set_ylim(0, max_value * 1.1)
            for bar in self.bars60:
                height = bar.get_height()
                self.ax60.text(bar.get_x() + bar.get_width()/2., height,
                            f'{height:.2f}',
                            ha='center', va='bottom')
            self.ax60.axhline(y=self.threshold * 100, color='r', linestyle='--', linewidth=1)
            self.ax60.text(1.01, self.threshold * 100, 'Threshold (%.1f%%)' % (self.threshold * 100), color='r', va='center', ha='left', 
    ...
    def process_and_update(self):
        try:
            ...
            for a in range(len(df)):
                ...
                self.serial_display.config(text=f"SERIAL No: {display_serial}")        # SERIAL NO AT 2ND TKINTER GUI
                self.model_display.config(text=f"MODEL CODE: {model_code}")
                self.serial_log.config(state='normal')
                self.serial_log.insert(tk.END, f"{display_serial}\n")
                ...
                if self.logic == "ACCU AVG (TOL 5%)":
                    ...
                    self.ref_serial_display.config(text=f"REF SERIAL NO: {ref_serial}")
    ...
    def update_display(self):
        try:
            ...
            if self.logic == "DUO (SINGLE NOZZLE) W/ 2 SERIAL NUMBER":
                ...
                display_serial = f"{serial_no}_{current_time}"
                self.serial_display.config(text=f"SERIAL No.: {display_serial}")
            else:
                serial_no = last_row["SERIAL No."]
    ...
    def create_section(self, title, columns):
        ...
        if title == "50Hz Measurements":
            self.section_50hz_frame = frame
        if title == "60Hz Measurements":
            self.section_60hz_frame = frame
    ...
    def open_display_settings(self):
        ...
        settings_window.bind_all("<MouseWheel>", lambda e: self._on_settings_mousewheel(e, canvas))
        settings_window.bind_all("<Button-4>", lambda e: self._on_settings_mousewheel(e, canvas))
        settings_window.bind_all("<Button-5>", lambda e: self._on_settings_mousewheel(e, canvas))
        ttk.Label(scrollable_frame, text="SPC LINE TREND WIDTH:").pack(pady=5)
        ...
        ttk.Label(scrollable_frame, text="X-AXIS:").pack(pady=5)
        ttk.Radiobutton(scrollable_frame, text="NUMERICAL NUMBERING", variable=self.xaxis_var, value="Numerical Numbering").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="MONTH/YEAR", variable=self.xaxis_var, value="Month/Year").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="SERIAL NO", variable=self.xaxis_var, value="Serial No").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="NONE", variable=self.xaxis_var, value="None").pack(anchor='w', padx=20)  # ADDED NONE OPTION
        ttk.Label(scrollable_frame, text="Layout:").pack(pady=5)
        ttk.Radiobutton(scrollable_frame, text="Vertical", variable=self.layout_var, value="Vertical").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="Horizontal", variable=self.layout_var, value="Horizontal").pack(anchor='w', padx=20)
        ttk.Checkbutton(scrollable_frame, text="SHOW TITTLE", variable=self.show_title_var).pack(anchor='w', padx=20, pady=5)
        ttk.Label(scrollable_frame, text="DATAPOINTS:").pack(pady=5)
        ttk.Radiobutton(scrollable_frame, text="SCATTER", variable=self.datapoints_var, value="Scatter").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="NONE", variable=self.datapoints_var, value="None").pack(anchor='w', padx=20)
        ttk.Label(scrollable_frame, text="SCATTER COLOR:").pack(pady=5)
        ttk.Radiobutton(scrollable_frame, text="NORMAL BLUE", variable=self.scatter_color_var, value="Normal Blue").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="NONE", variable=self.scatter_color_var, value="None").pack(anchor='w', padx=20)
        ttk.Label(scrollable_frame, text="Sample Number:").pack(pady=5)
        ttk.Radiobutton(scrollable_frame, text="3", variable=self.sample_number_var, value="3").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="5", variable=self.sample_number_var, value="5").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="Others", variable=self.sample_number_var, value="others").pack(anchor='w', padx=20)
        self.sample_other_frame = ttk.Frame(scrollable_frame)
        ttk.Label(self.sample_other_frame, text="Enter Number:").pack(side=tk.LEFT)
        self.sample_entry = ttk.Entry(self.sample_other_frame, textvariable=self.sample_entry_var, width=5)
        self.sample_entry.pack(side=tk.LEFT)
        self.sample_number_var.trace("w", self.on_sample_change)
        self.on_sample_change()
        ttk.Label(scrollable_frame, text="LEGEND:").pack(pady=5)
        ttk.Radiobutton(scrollable_frame, text="YES", variable=self.legend_var, value="YES").pack(anchor='w', padx=20)
        ttk.Radiobutton(scrollable_frame, text="NO", variable=self.legend_var, value="NO").pack(anchor='w', padx=20)
        ttk.Button(scrollable_frame, text="Apply", command=lambda: [self.update_line_graph(), settings_window.destroy(), self.rebind_mousewheel()]).pack(pady=10)

    def _on_settings_mousewheel(self, event, canvas):
        delta = 0
        if event.num == 4:
            delta = 120
        elif event.num == 5:
    ...
    def on_sample_change(self, *args):
        if self.sample_number_var.get() == "others":
            self.sample_other_frame.pack(anchor='w', padx=40, pady=5)
        else:
            self.sample_other_frame.pack_forget()

    def open_all_spc_charts(self):
        measurements_50 = ["50Hz WATTAGE", "50Hz AIR VOLUME", "50Hz CLOSED PRESSURE", "50Hz AMPERAGE"]
        measurements_60 = ["60Hz WATTAGE", "60Hz AIR VOLUME", "60Hz CLOSED PRESSURE", "60Hz AMPERAGE"]
        df = self.compiledFrame
        if df.empty:
            return
        has_50 = any(df[m].sum() > 0 for m in measurements_50)
        has_60 = any(df[m].sum() > 0 for m in measurements_60)
        if not has_50 and not has_60:
            return
        to_open = []
        if has_50:
            to_open += measurements_50
        if has_60:
            to_open += measurements_60
        for meas in to_open:
            self.show_separate_graph(meas)

    def show_separate_graph(self, selected):
        graph_window = tk.Toplevel(self.root)
        graph_window.title(f"SPC: {selected}")
        graph_window.geometry("1200x800")
        fig = Figure(figsize=(12, 8), dpi=100)
        ax = fig.add_subplot(111)
        ax.set_title(f'{selected} Trend')
        ...
        if hasattr(self, 'compiledFrame') and not self.compiledFrame.empty:
            ...
            ucl = mean + 3 * std
            lcl = mean - 3 * std
            ax.plot(df['DATETIME'], y, label=selected, color=color_map.get(selected, 'blue'), linewidth=2)
            ax.axhline(mean, color=color_map.get(selected, 'blue'), linestyle='-', alpha=0.5)
            ax.axhline(ucl, color=color_map.get(selected, 'blue'), linestyle='--', alpha=0.5)
            ax.axhline(lcl, color=color_map.get(selected, 'blue'), linestyle='--', alpha=0.5)
            ax.legend()
            ax.tick_params(axis='x', rotation=45, labelsize=10)
        fig.subplots_adjust(left=0.1, right=0.9, bottom=0.15, top=0.9)
        canvas = FigureCanvasTkAgg(fig, master=graph_window)
    ...
    def open_hide_selection(self):
        hide_window = tk.Toplevel(self.root)
        hide_window.title("Hide Sections")
        hide_window.geometry("300x200")
        ttk.Label(hide_window, text="Hide Process Inspections:", font=('Arial', 12)).pack(pady=10)
        ttk.Checkbutton(hide_window, text="50HZ PROCESS INSPECTION", variable=self.hide_50hz, command=self.apply_hides).pack(anchor='w', padx=20)
        ttk.Checkbutton(hide_window, text="60HZ PROCESS INSPECTION", variable=self.hide_60hz, command=self.apply_hides).pack(anchor='w', padx=20)
        ttk.Button(hide_window, text="Close", command=hide_window.destroy).pack(pady=10)

    def apply_hides(self):
        self.section_50hz_frame.pack_forget()
        self.sep_after_50.pack_forget()
        self.section_60hz_frame.pack_forget()
        self.sep_after_60.pack_forget()
        if self.avg_frame:
            self.avg_frame.pack_forget()
        if not self.hide_50hz.get():
            self.section_50hz_frame.pack(fill=tk.X, pady=2)
            self.sep_after_50.pack(fill='x', pady=5)
        if not self.hide_60hz.get():
            self.section_60hz_frame.pack(fill=tk.X, pady=2)
            self.sep_after_60.pack(fill='x', pady=5)
        if self.avg_frame:
            self.avg_50hz_label.pack_forget()
            for r in self.avg_50hz_rows:
                r.pack_forget()
            self.avg_60hz_label.pack_forget()
            for r in self.avg_60hz_rows:
                r.pack_forget()
            if not self.hide_50hz.get():
                self.avg_50hz_label.pack(anchor='w')
                for r in self.avg_50hz_rows:
            ...
            if not self.hide_60hz.get():
                ...
                for r in self.avg_60hz_rows:
                    r.pack(fill=tk.X)
            self.avg_frame.pack(fill=tk.X, pady=5)
        self.canvas_graph50.get_tk_widget().pack_forget()
        self.canvas_graph60.get_tk_widget().pack_forget()
        if not self.hide_50hz.get():
            self.canvas_graph50.get_tk_widget().pack(fill=tk.X, expand=False, padx=10, pady=5)
        if not self.hide_60hz.get():
            self.canvas_graph60.get_tk_widget().pack(fill=tk.X, expand=False, padx=10, pady=5)
